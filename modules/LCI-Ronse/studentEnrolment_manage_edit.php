...
if (isActionAccessible($guid, $connection2, '/modules/Students/studentEnrolment_manage_edit.php') == false) {
    ...
    $_SESSION[$guid]['module'] = 'Students';
    $page->breadcrumbs
        ->setBaseURL('index.php?q=/modules/'.$_SESSION[$guid]['module'].'/')
    ...
    if ($gibbonStudentEnrolmentID == '' or $gibbonSchoolYearID == '') {
        ...
        try {
            ...
            $sql = 'SELECT gibbonRollGroup.gibbonRollGroupID, gibbonYearGroup.gibbonYearGroupID,gibbonStudentEnrolmentID, surname, preferredName, gibbonYearGroup.nameShort AS yearGroup, gibbonRollGroup.nameShort AS rollGroup, dateStart, dateEnd, gibbonPerson.gibbonPersonID, gibbonPerson.nameInCharacters, rollOrder FROM gibbonPerson, gibbonStudentEnrolment, gibbonYearGroup, gibbonRollGroup WHERE (gibbonPerson.gibbonPersonID=gibbonStudentEnrolment.gibbonPersonID) AND (gibbonStudentEnrolment.gibbonYearGroupID=gibbonYearGroup.gibbonYearGroupID) AND (gibbonStudentEnrolment.gibbonRollGroupID=gibbonRollGroup.gibbonRollGroupID) AND gibbonRollGroup.gibbonSchoolYearID=:gibbonSchoolYearID AND gibbonStudentEnrolmentID=:gibbonStudentEnrolmentID ORDER BY surname, preferredName';
        ...
        if ($result->rowCount() != 1) {
            ...
            $row = $form->addRow();
                ...
                $row->addTextField('studentName')->readOnly()->setValue(Format::name('', $values['preferredName'].', '.$values['nameInCharacters'], $values['surname'], 'Student', true));
            ...
            $row = $form->addRow();
                ...
                $data = array('gibbonSchoolYearID' => $gibbonSchoolYearID);
                $sql = customGbn_getSqlRollGroup();
                $row->addSelect('gibbonRollGroupID')->fromQueryChained($pdo, $sql, $data)->required()->placeholder();
