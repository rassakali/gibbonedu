...
if (isActionAccessible($guid, $connection2, '/modules/Students/applicationForm_manage_accept.php') == false) {
    ...
    $page->breadcrumbs->add(__('Manage Applications'), 'applicationForm_manage.php', [
    ...
    if ($gibbonApplicationFormID == '' or $gibbonSchoolYearID == '') {
        ...
        if ($result->rowCount() != 1) {
            ...
            if ($resultStudent) {
                $studentApplication = $resultStudent->fetch();
                $gibbonPersonID = $studentApplication['gibbonPersonID'];
            }
            ...
            if ($step == 1) {
                ...
                if (! empty($gibbonPersonID)) {
                    $list->append('<li>' . __('Student already exist') . ': ' . $gibbonPersonID . ' - ' . $studentApplication['preferredName'] . ' ' . $studentApplication['surname'] . ' (' . $studentApplication['nationalIDCardNumber'] . ')</li>');
                } else {
                ...
                if (! empty($values['gibbonFamilyID']) || ! empty($linkedApplication['gibbonFamilyID'])) {
                    $list->append('<li>' . __('Link student to family (who are already in Gibbon).') . ': ' . $linkedApplication['gibbonFamilyID'] . ' - ' . $linkedApplication['familyName'] . '</li>');
                } else {
                ...
                $list->append('<li>' . __('Save the student\'s payment preferences.') . '</li>')->append('<li>' . __('Set the status of the application to "Accepted".') . '</li>');
                ...
                if (empty($studentApplication) && ! isset($gibbonPersonID)) {
                    // CREATE STUDENT
                    $failStudent = true;
                    $lock = true;
                    try {
                        ...
                        if ($gotAI == true) {
                            ...
                            try {
                                $dataSchoolYear = array(
                                    'gibbonSchoolYearID' => $values['gibbonSchoolYearIDEntry']
                                );
                            ...
                            try {
                                $dataYearGroup = array(
                                    'gibbonYearGroupID' => $values['gibbonYearGroupIDEntry']
                                );
                            ...
                            if ($continueLoop == false) {
                                ...
                                try {
                                    $data = array(
                                        ...
                                        'nameInCharacters' => $values['nameInCharacters'],
                                        'gender' => $values['gender'],
                                        'dob' => $values['dob'],
                                        'languageFirst' => $values['languageFirst'],
                                        'languageSecond' => $values['languageSecond'],
                                        'languageThird' => $values['languageThird'],
                                        'countryOfBirth' => $values['countryOfBirth'],
                                        'citizenship1' => $values['citizenship1'],
                                        'citizenship1Passport' => $values['citizenship1Passport'],
                                        'nationalIDCardNumber' => $values['nationalIDCardNumber'],
                                        'residencyStatus' => $values['residencyStatus'],
                                        'visaExpiryDate' => $values['visaExpiryDate'],
                                        'email' => $email,
                                        'emailAlternate' => $emailAlternate,
                                        'website' => $website,
                                        'phone1Type' => $values['phone1Type'],
                                        'phone1CountryCode' => $values['phone1CountryCode'],
                                        'phone1' => $values['phone1'],
                                        'phone2Type' => $values['phone2Type'],
                                        'phone2CountryCode' => $values['phone2CountryCode'],
                ...
                if ($failStudent == true) {
                    ...
                    if (empty($studentApplication)) {
                        ...
                        echo '<li><b>' . __('Name') . '</b>: ' . Format::name('', $studentApplication['preferredName'], $studentApplication['surname'], 'Student') . '</li>';
                    }
                    echo '</ul>';
                    ...
                    if (! empty($values['gibbonFamilyID']) || ! empty($linkedApplication['gibbonFamilyID'])) {
                        ...
                        if ($resultFamilyChildLink->rowCount() == 0) {
                            try {
                                $dataFamily = array(
                                    'gibbonFamilyID' => $values['gibbonFamilyID']
                                );
                            ...
                            for ($i = 1; $i <= 2; $i ++) {
                                ...
                                if (empty($values["parent{$i}gibbonPersonID"])) {
                                    try {
                                        $dataParent = array(
                                            'gibbonFamilyID' => $values['gibbonFamilyID'],
                                            'parentSurname' => $values["parent{$i}surname"],
                                            'parentPreferredName' => $values["parent{$i}preferredName"]
                                        );
                            ...
                            if (strpos($rowParents['gibbonRoleIDAll'], '004') === false) {
                                try {
                                    $dataRoleUpdate = array(
                                        'gibbonPersonID' => $rowParents['gibbonPersonID']
                                    );
                                ...
                                if ($result->rowCount() == 0) {
                                    ...
                                    if ($existingRelationship['relationship'] != $relationship) {
                                        try {
                                            $data = array(
                                                'relationship' => $relationship,
                                                'gibbonFamilyRelationshipID' => $existingRelationship['gibbonFamilyRelationshipID']
                                            );
                            ...
                            if ($gotAI == true) {
                                ...
                                try {
                                    $data = array(
                                        ...
                                        'languageHomePrimary' => $languageHomePrimary,
                                        'languageHomeSecondary' => $languageHomeSecondary,
                                        'homeAddress' => $values['homeAddress'],
                                        'homeAddressDistrict' => $values['homeAddressDistrict'],
                                        'homeAddressCountry' => $values['homeAddressCountry']
                                    );
                                        ...
                                        if ($continueLoop == false) {
                                            ...
                                            try {
                                                $data = array(
                                                    ...
                                                    'passwordStrong' => $passwordStrong,
                                                    'passwordStrongSalt' => $salt,
                                                    'title' => $values['parent1title'],
                                                    'surname' => $values['parent1surname'],
                                                    'firstName' => $values['parent1firstName'],
                                                    'preferredName' => $values['parent1preferredName'],
                                                    'officialName' => $values['parent1officialName'],
                                                    'nameInCharacters' => $values['parent1nameInCharacters'],
                                                    'gender' => $values['parent1gender'],
                                                    'parent1languageFirst' => $values['parent1languageFirst'],
                                                    'parent1languageSecond' => $values['parent1languageSecond'],
                                                    'citizenship1' => $values['parent1citizenship1'],
                                                    'nationalIDCardNumber' => $values['parent1nationalIDCardNumber'],
                                                    'residencyStatus' => $values['parent1residencyStatus'],
                                                    'visaExpiryDate' => $values['parent1visaExpiryDate'],
                                                    'email' => $values['parent1email'],
                                                    'phone1Type' => $values['parent1phone1Type'],
                                                    'phone1CountryCode' => $values['parent1phone1CountryCode'],
                                                    'phone1' => $values['parent1phone1'],
                                                    'phone2Type' => $values['parent1phone2Type'],
                                                    'phone2CountryCode' => $values['parent1phone2CountryCode'],
                                                    'phone2' => $values['parent1phone2'],
                                                    'profession' => $values['parent1profession'],
                                                    'employer' => $values['parent1employer'],
                                                    'parent1fields' => $values['parent1fields']
                                                );
                                        ...
                                        if ($continueLoop == false) {
                                            ...
                                            try {
                                                $data = array(
                                                    ...
                                                    'passwordStrong' => $passwordStrong,
                                                    'passwordStrongSalt' => $salt,
                                                    'title' => $values['parent2title'],
                                                    'surname' => $values['parent2surname'],
                                                    'firstName' => $values['parent2firstName'],
                                                    'preferredName' => $values['parent2preferredName'],
                                                    'officialName' => $values['parent2officialName'],
                                                    'nameInCharacters' => $values['parent2nameInCharacters'],
                                                    'gender' => $values['parent2gender'],
                                                    'parent2languageFirst' => $values['parent2languageFirst'],
                                                    'parent2languageSecond' => $values['parent2languageSecond'],
                                                    'citizenship1' => $values['parent2citizenship1'],
                                                    'nationalIDCardNumber' => $values['parent2nationalIDCardNumber'],
                                                    'residencyStatus' => $values['parent2residencyStatus'],
                                                    'visaExpiryDate' => $values['parent2visaExpiryDate'],
                                                    'email' => $values['parent2email'],
                                                    'phone1Type' => $values['parent2phone1Type'],
                                                    'phone1CountryCode' => $values['parent2phone1CountryCode'],
                                                    'phone1' => $values['parent2phone1'],
                                                    'phone2Type' => $values['parent2phone2Type'],
                                                    'phone2CountryCode' => $values['parent2phone2CountryCode'],
                                                    'phone2' => $values['parent2phone2'],
                                                    'profession' => $values['parent2profession'],
                                                    'employer' => $values['parent2employer'],
                                                    'parent2fields' => $values['parent2fields']
                                                );
