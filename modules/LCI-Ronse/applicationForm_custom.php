...
if ($proceed == false) {
    ...
    if (! empty($_GET['id'])) {
        ...
        $data = array(
            'gibbonApplicationFormHash' => $_GET['id']
        );
    ...
    $form = Form::create('applicationForm', $_SESSION[$guid]['absoluteURL'] . '/modules/' . $_SESSION[$guid]['module'] . '/applicationForm_customProcess.php');
    ...
    $row->addLabel('dob', __('Date of Birth'))
        ->description($_SESSION[$guid]['i18n']['dateFormat'])
        ->prepend(__('Format:'));
    ...
    $row->addTextField('nationalIDCardNumber')
        ->required()
        ->maxLength(30);
    ...
    $form->addHiddenValue('languageHomePrimary', 'French');
    ...
    $form->addHiddenValue('countryOfBirth', 'Inconnu');
    ...
    // }
    $form->addHiddenValue('citizenship1', 'Inconnu');
    ...
    if ($enableLimitedYearsOfEntry == 'Y' && ! empty($availableYearsOfEntry)) {
        $data = array(
            'gibbonSchoolYearIDList' => $availableYearsOfEntry
        );
    ...
    if (! empty($availableYearsOfEntry)) {
        $arrTmp = explode(',', $availableYearsOfEntry);
        $gibbonSchoolYearIDEntryInit = $arrTmp[0];
    } else {
        $gibbonSchoolYearIDEntryInit = '';
    }
    $row->addSelect('gibbonSchoolYearIDEntry')
        ->fromQuery($pdo, $sql, $data)
        ->required()
        ->selected($gibbonSchoolYearIDEntryInit)
        ->placeholder(__('Please select...'));

    // $row = $form->addRow();
    // $row->addLabel('dateStart', __('Intended Start Date'))
    // ->description(__('Student\'s intended first day at school.'))
    // ->append('<br/>' . __('Format:'))
    // ->append(' ' . $_SESSION[$guid]['i18n']['dateFormat']);
    ...
    $row->addSelect('gibbonYearGroupIDEntry')
        ->fromQuery($pdo, $sql)
        ->required()
        ->selected('017')
        ->placeholder(__('Please select...'));
    ...
    if (false) {
        ...
        if ($senOptionsActive == 'Y') {
            ...
            $form->toggleVisibilityByClass('senDetailsRow')
                ->onSelect('sen')
                ->when('Y');
        ...
        for ($i = 1; $i < 3; ++ $i) {
            $row = $table->addRow();
            $row->addTextField('schoolName' . $i)
                ->maxLength(50)
                ->setSize(18);
            $row->addTextField('schoolAddress' . $i)
                ->maxLength(255)
                ->setSize(20);
            $row->addTextField('schoolGrades' . $i)
                ->maxLength(20)
                ->setSize(8);
            $row->addTextField('schoolLanguage' . $i)
        ...
        for ($i = 1; $i < 3; ++ $i) {
            ...
            $form->addHiddenValue('schoolLanguage' . $i, '');
            $form->addHiddenValue('schoolDate' . $i, '');
        }
    }
    ...
    if ($public == true or $resultSelect->rowCount() < 1) {
        ...
        if ($siblingApplicationMode == true) {
            ...
            // HOME ADDRESS
            $form->addRow()
                ->addHeading(__('Home Address'))
                ->append(__('This address will be used for all members of the family. If an individual within the family needs a different address, this can be set through Data Updater after admission.'));

            $row = $form->addRow();
            $row->addLabel('homeAddress', __('Home Address'))->description(__('Unit, Building, Street'));
            $row->addTextArea('homeAddress')
                ->required()
                ->maxLength(255)
                ->setRows(2);
        ...
        if (! empty($gibbonPersonID) || ! empty($application['parent1gibbonPersonID'])) {
            ...
            $dataParent = array(
                'gibbonPersonID' => $parent1gibbonPersonID
            );
            ...
            $row->addTextField('parent1username')
                ->setValue($parent1username)
                ->maxLength(30)
                ->readOnly();

            $row = $form->addRow();
            $row->addLabel('parent1surname', __('Surname'))->description(__('Family name as shown in ID documents.'));
            $row->addTextField('parent1surname')
                ->setValue($parent1surname)
                ->maxLength(30)
                ->readOnly();
        ...
        for ($i = $start; $i < 3; ++ $i) {
            ...
            $form->addRow()
                ->addHeading(__('Parent/Guardian') . ' ' . $i)
                ->append($subheading);

            if ($i == 2) {
                $checked = ($siblingApplicationMode && ! empty($application['parent2gibbonPersonID'])) ? 'Yes' : 'No';
                $form->addRow()
                    ->addCheckbox('secondParent')
                    ->setValue('No')
                    ->checked($checked)
                    ->prepend(__('Do not include a second parent/guardian'));
                $form->toggleVisibilityByClass('parentSection2')
                    ->onCheckbox('secondParent')
                    ->whenNot('No');
            }
            ...
            $row->addTextField("parent{$i}firstName")
                ->required()
                ->maxLength(30)
                ->loadFrom($application);
            ...
            $form->addHiddenValue("parent{$i}preferredName", '');
            ...
            $form->addHiddenValue("parent{$i}officialName", '');

            $row = $form->addRow()->setClass("parentSection{$i}");
            $row->addLabel("parent{$i}gender", __('Gender'));
            $row->addSelectGender("parent{$i}gender")
                ->required()
                ->loadFrom($application);
            ...
            // }
            $form->addHiddenValue("parent{$i}citizenship1", "");
            ...
            for ($y = 1; $y < 2; ++ $y) {
                if (! isset($application["parent{$i}phone{$y}Type"]) && ! isset($application["parent{$i}phone{$y}CountryCode"])) {
                    $application["parent{$i}phone{$y}Type"] = "Mobile";
                    $application["parent{$i}phone{$y}CountryCode"] = "32";
                }
                $row = $form->addRow()->setClass("parentSection{$i}");
                $row->addLabel("parent{$i}phone{$y}", __('Phone') . ' ' . $y)->description(__('Type, country code, number.'));
                $row->addPhoneNumber("parent{$i}phone{$y}")
                    ->setRequired($i == 1 && $y == 1)
                    ->loadFrom($application);
            }

            // $form->addHiddenValue("parent{$i}email", "N/C");
            $row = $form->addRow()->setClass("parentSection{$i}");
            $row->addLabel("parent{$i}email", __('Email'));
            $email = $row->addEmail("parent{$i}email")->loadFrom($application);
            if ($uniqueEmailAddress == 'Y') {
                $email->uniqueField('./publicRegistrationCheck.php', array(
                    'fieldName' => 'email'
                ));
            }
            ...
            $row->addTextField("parent{$i}nationalIDCardNumber")
                ->maxLength(30)
                ->loadFrom($application);
            ...
            $row->addSelectLanguage("parent{$i}languageFirst")
                ->placeholder()
                ->loadFrom($application);
            ...
            $form->addHiddenValue("parent{$i}profession", "N/A");
            // $row = $form->addRow()->setClass("parentSection{$i}");
        ...
        while ($rowSelect = $resultSelect->fetch()) {
            ...
            $row->addContent($rowSelect['name'])
                ->wrap('<strong>', '</strong>')
                ->addClass('shortWidth');
            $row->addRadio('gibbonFamilyID')
                ->fromArray(array(
                $rowSelect['gibbonFamilyID'] => ''
            ))
                ->checked($checked);
            ...
            while ($rowRelationships = $resultRelationships->fetch()) {
                ...
                $subTableRow->addSelectRelationship($rowSelect['gibbonFamilyID'] . '-relationships[]')
                    ->selected($selected)
    ...
    if (false) {
        $form->addRow()
            ->addHeading(__('Siblings'))
            ->append(__('Please give information on the applicants\'s siblings.'));
        ...
        if (isset($gibbonFamilyID)) {
            ...
            while ($rowSibling = $resultSibling->fetch()) {
                ...
                $row = $table->addRow();
                $row->addTextField('siblingName' . $rowCount)
                    ->maxLength(50)
                    ->setSize(26)
                    ->setValue($name);
                $row->addDate('siblingDOB' . $rowCount)
                    ->setSize(10)
                    ->setValue(dateConvertBack($guid, $rowSibling['dob']));
                $row->addTextField('siblingSchool' . $rowCount)
                    ->maxLength(50)
                    ->setSize(30)
                    ->setValue($_SESSION[$guid]['organisationName']);
                $row->addDate('siblingSchoolJoiningDate' . $rowCount)
                    ->setSize(10)
                    ->setValue(dateConvertBack($guid, $rowSibling['dateStart']));
        ...
        for ($i = $rowCount; $i <= 3; ++ $i) {
            $row = $table->addRow();
            $nameField = $row->addTextField('siblingName' . $i)
                ->maxLength(50)
                ->setSize(26);
            $dobField = $row->addDate('siblingDOB' . $i)->setSize(10);
            $row->addTextField('siblingSchool' . $i)
                ->maxLength(50)
                ->setSize(30);
        ...
        for ($i = 1; $i < 4; ++ $i) {
            $form->addHiddenValue('siblingName' . $i, '');
            $form->addHiddenValue('siblingDOB' . $i, '');
            $form->addHiddenValue('siblingSchool' . $i, '');
            $form->addHiddenValue('siblingSchoolJoiningDate' . $i, '');
            $form->addHiddenValue('schoolLanguage' . $i, '');
            $form->addHiddenValue('schoolDate' . $i, '');
        }
    }
    ...
    if ($languageOptionsActive == 'Y' && $languageOptionsLanguageList != '') {
        ...
        $column->addTextArea('languageChoiceExperience')
            ->required()
            ->setRows(5)
            ->setClass('fullWidth');
    }
    ...
    if ($scholarshipOptionsActive == 'Y') {
        ...
        $row->addRadio('scholarshipInterest')
            ->fromArray(array(
            'Y' => __('Yes'),
            'N' => __('No')
        ))
        ...
        $row->addRadio('scholarshipRequired')
            ->fromArray(array(
            'Y' => __('Yes'),
            'N' => __('No')
        ))
    ...
    $row = $form->addRow();
    $column = $row->addColumn();
    $column->addLabel('', __('Notes'));
    $column->addTextArea('notes')
    ->setRows(5)
    ->setClass('fullWidth');
    ...
    if ($paymentOptionsActive == 'Y') {
        ...
        $form->toggleVisibilityByClass('paymentCompany')
            ->onRadio('payment')
            ->when('Company');
        ...
        $row->addTextField('companyContact')
            ->required()
            ->maxLength(100)
            ->loadFrom($application);
        ...
        $row->addTextField('companyAddress')
            ->required()
            ->maxLength(255)
            ->loadFrom($application);
        ...
        if (! $resultFees || $resultFees->rowCount() == 0) {
            ...
            $form->toggleVisibilityByClass('paymentCompanyCategories')
                ->onRadio('companyAll')
                ->when('N');
            ...
            $row = $form->addRow()
                ->addClass('paymentCompany')
                ->addClass('paymentCompanyCategories');
    ...
    // MISCELLANEOUS
    $form->addHiddenValue('howDidYouHear', 'Advertisement');
    $form->addHiddenValue('tellUsMore', 'N/C');
    ...
    // $row->addTextField('howDidYouHear')
    // ->required()
    // ->maxLength(30)
    // ->loadFrom($application);
    // } else {
    // $row->addSelect('howDidYouHear')
    // ->fromArray($howDidYouHearList)
    // ->required()
    // ->placeholder()
    // ->loadFrom($application);

    // $form->toggleVisibilityByClass('tellUsMore')
    // ->onSelect('howDidYouHear')
    // ->whenNot(__('Please select...'));
    ...
    // $row->addTextField('howDidYouHearMore')
    // ->maxLength(255)
    // ->loadFrom($application);
    // }
    ...
    if ($privacySetting == 'Y' && ! empty($privacyBlurb) && ! empty($privacyOptions)) {
        ...
        $form->addRow()
            ->addSubheading(__('Privacy'))
            ->append($privacyBlurb);

        $options = array_map(function ($item) {
            return trim($item);
        }, explode(',', $privacyOptions));

        $row = $form->addRow();
        $row->addLabel('privacyOptions[]', __('Privacy Options'));
        $row->addCheckbox('privacyOptions[]')
            ->fromArray($options)
            ->addClass('md:max-w-lg');
    }
    ...
    if (isActionAccessible($guid, $connection2, '/modules/Students/applicationForm_manage.php')) {
        ...
        $row->addCheckbox('skipEmailNotification')
            ->description(__('Yes'))
            ->setValue('on')
            ->checked('on');
    }
